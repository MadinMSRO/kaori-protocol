...
Before compilation, the API sends the accepted observation batch to the private `kaori-generalist` service and records its signed `VALIDATION_VOTE` as `ai:generalist_v1`. Only then does it call the pure compiler. Compile 200 upserts the full `TruthState.model_dump` (including `evidence_refs`) into `kaori.truth_states`, then calls `FlowCore.emit_truthstate` with the Bearer agent as the sole contributor. Standing moves from that signal history — compile does **not** call `register_agent` for the Bearer agent. On startup the sidecar registers Flow agent `ai:generalist_v1` (role `validator`) if unknown (idempotent).

The separate IAM-protected `kaori-generalist` Cloud Run service loads that ClaimType's existing YAML and runs one open CLIP generalist on CPU, comparing the evidence with `ai_validation_routing.generalist.prompt_context` (or the schema's display/topic fallback) versus unrelated imagery. It compares the mean relevance probability directly with `evidence_similarity.embedding.similarity_threshold`, signs a FLOW_SPEC `ValidationSignal` as `ai:generalist_v1`, and returns it. Only `kaori-api` calls `record_validation_vote`, so it remains the single SignalStore writer. The compiler remains pure: it never invokes the service, executes a validator, or writes a `VALIDATION_VOTE`.

The precompile gate fails closed. `REJECT` is recorded and returns HTTP 422 without compiling, persisting, or emitting a TruthState. `ABSTAIN` is recorded and compilation continues with `VALIDATOR_ABSTAINED` in `transparency_flags`. A missing or failed generalist returns HTTP 503 before compilation. For an accepted vote, status is derived generically from ClaimType YAML: `human_gating.always_require_human`, `human_gating.required_for_risk_profiles`, `risk_profile`, and `validation_flow.mode`. Human-gated lanes use `PENDING_HUMAN_REVIEW`; an auto lane remains at the existing intermediate `PENDING` status while the validation window is open. There is no `VALIDATION` TruthStatus.

Submission checks remain in the existing compile/submit 400 path and are not duplicated in Cloud Run. The model service receives only `truthkey_id`, `claim_type_id`, and `evidence_refs`; it does not receive GPS, depth, or observation payload fields. Public `http://` and `https://` evidence (including Supabase public object URLs) is fetched directly; `gs://` evidence continues to use the Cloud Run service account. No new bucket is required. The service does not run submission rules, pHash, a specialist, a chat LLM, Vertex AI, or a GPU. Coral still has `always_require_human: true`, so an accepted compile remains at `PENDING_HUMAN_REVIEW`.
