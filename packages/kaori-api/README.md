...
Compile records each Bearer-stamped Observation on the append-only `kaori.observations` ledger (one immutable Observation per reporter per TruthKey; same hash is idempotent; a mutated resubmit is 400) and compiles every ledger Observation for that TruthKey. No fourth path. Compile 200 upserts the full `TruthState.model_dump` (including `evidence_refs`) into `kaori.truth_states`, appends `kaori.artifact_ledger`, then calls `FlowCore.emit_truthstate` with the Bearer agent and Flow agent `claimtype:{claim_type_id}` as contributors. Standing moves from that signal history — compile does **not** call `register_agent` for the Bearer agent. On startup the sidecar registers Flow agent `ai:generalist_v1` (role `validator`) if unknown (idempotent). On compile it idempotently registers `claimtype:{claim_type_id}` (FLOW_SPEC Rule 2 role `claimtype`) if unknown. Claim-type rank uses the same `GET /v1/standing/{agent_id}` with `claimtype:{claim_type_id}` encoded in the path. Same Bearer. Response stays `{ "standing": <number> }`. Player standing stays `user:{id}`. `GET /v1/truth/{truthkey}` still returns the TruthState artifact including `claim`, `compile_inputs.observations` (canonical geo, payload, and content-bound `{uri, sha256}` evidence_refs), and `consensus.votes` when a `VALIDATION_VOTE` was recorded. No `GET /v1/vote`. No fourth path.
...
2. Apply schema (or let the sidecar call `ensure_schema()` on boot). That creates schema `kaori` and tables `kaori.signals`, `kaori.truth_states`, `kaori.observations`, and `kaori.artifact_ledger` — never `public.signals` or `public.truths`:
...
When `DATABASE_URL` is set, `FlowCore` uses `PostgresSignalStore` (`kaori.signals`), compile persist uses `PostgresTruthStateStore` (`kaori.truth_states`), and Observation intake / artifact history uses `PostgresArtifactLedger`. Without it, the sidecar uses in-memory stores.
