...
Auth: `Authorization: Bearer <token>` verified by `GET {SUPABASE_URL}/auth/v1/user` with `apikey: SUPABASE_PUBLISHABLE_KEY`. Agent id is `user:{user.id}`. Never `profiles.id`. The sidecar stamps `Observation.reporter_id` from the Bearer agent and `reporter_context` from Flow. Startup idempotently registers the CPU CLIP voter `ai:generalist_v1` with role validator. After submission checks, the private model service loads the ClaimType YAML and returns a signed relevance vote; `kaori-api` records it through the internal `record_validation_vote` helper before calling `compile_truth_state`. The helper is not a route, and the compiler remains pure. `REJECT` returns 422 before compile; `ABSTAIN` still compiles with a transparency flag; validator unavailability returns 503. Compile 200 upserts `TruthState.model_dump` into `kaori.truth_states` (not `public.truths`) then emits `FlowCore.emit_truthstate` so standing moves from that signal — not from `register_agent` for the Bearer. Status comes from ClaimType `human_gating`, `risk_profile`, and `validation_flow`: human-gated claims use `PENDING_HUMAN_REVIEW`, while an auto lane remains `PENDING` during validation. No `VALIDATION` status exists.
