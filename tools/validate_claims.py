...
SCHEMA_PATH = ROOT / "packages" / "kaori-spec" / "schemas" / "meta" / "claimtype.schema.json"
SCHEMAS_DIR = ROOT / "packages" / "kaori-spec" / "schemas"
LOCKFILE_PATH = ROOT / "packages" / "kaori-spec" / "claimtype.lock.json"
...
def enforce_immutability(claim_path: Path, lock_data: dict):
    ...
    # For relative path, use path relative to SCHEMAS_DIR or ROOT?
    # Old script used ROOT. Since we changed ROOT structure, let's stick to ROOT relative to ensure uniqueness.
    try:
        rel = str(claim_path.relative_to(ROOT))
    except ValueError:
        # Should not happen if SCHEMAS_DIR is child of ROOT
        rel = str(claim_path)
...
def main():
    ...
    print("[OK] All claim schemas validated successfully.")
    sys.exit(0)
